...
def main(img1, img2, kp1,des1,kp2,des2, get_des_from_kp,threshold = 550,th1=0.6,th2=0.3,func = 2):
    ...
    matched_pts_src, matched_pts_dst, m_arr = multi_homography.get_multi_homography(pts_src, pts_dst,outlier_proportion=th2)
    ...
    if (func == 1) | (func == 2):
        ...
        for i in range(len(matched_pts_src)):
            ...
            # KNN训练数据
            if(i==0):
                train_data.append(matched_src[0][0])
                tmp_src = np.reshape(matched_src,(-1,2))
                train_data = np.concatenate((train_data,tmp_src[1:]))
                train_labels.append([i])
                train_labels = np.concatenate((train_labels,[[i]] * (len(matched_src)-1)))
            else:
                tmp_src = np.reshape(matched_src, (-1, 2))
                train_data = np.concatenate((train_data, tmp_src[:]))
                train_labels = np.concatenate((train_labels, [[i]] * len(matched_src)))
        ...
        _, result, _, _ = knn.findNearest(np.reshape(demo_src, (-1, 2)), 5)
        result = np.array(result)
        region_points = [[] for i in range(len(regions))]
        for i in range(len(demo_src)):
            region_points[int(result[i])].append(demo_src[i])
